...
Status: **Provisional design contract; mocked only**

This contract describes the one-way data Franky Presence needs from the
computer runtime. It is not an accepted architecture decision and is not yet a
live protocol. The current page consumes deterministic local mock events only.
...
| Field | Type | Meaning |
| --- | --- | --- |
| `version` | integer | Contract version. The mock consumer accepts only `1`. |
...
9. On a future feed disconnect, enter offline after a bounded runtime-defined
   grace period. On reconnect, require a new authoritative snapshot before
   showing idle or turn content.

## Security and privacy boundary

The production surface must remain a one-way consumer. The intended runtime
route is read-only and loopback or explicitly authenticated for its deployment
context. Franky Presence must not gain POST endpoints, serial access, tool
execution, command controls, retries, settings, or model prompts.
